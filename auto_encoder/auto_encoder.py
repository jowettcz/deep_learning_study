...
class AdditiveGaussianNoiseAutoencoder(object):
    def __init__(self,n_input,n_hidden,transfer_function=tf.nn.softplus,
        ...
        self.x = tf.placeholder([None,n_input])
        self.hidden = self.transfer(tf.add(tf.matmul(
                           self.x + tf.random_normal((n_input,)),
    ...
    def generate(self,hidden = None):
        if hidden is None:
            hidden = np.random.normal(size=self.weights['b1'].shape)  #error?
...
def get_radom_block_from_data(data,batch_size):
...
for epoch in xrange(training_epochs):
    ...
    for i in xrange(total_batch):
        batch_xs = get_radom_block_from_data(X_train,batch_size)
